...
trait Helpers {
    ...
    public function getUserLoginInformation(string $ip): array
    {
        $info = [];
    ...
    public function requestAttempts(Request $request, string $trans = 'auth.throttle'): ?array
    {
    ...
    public function userLoginNotification(User $user, string $ip): void
    {
        if (config('api.login.notify.user', true)) {
            $time = Carbon::now()->isoFormat('Do of MMMM YYYY, h:mm:ssa');
            $deviceAndLocation = $this->getUserLoginInformation($ip);
            $url = (new $user->model)->url();
            $user->notify(new UserLogin($time, $deviceAndLocation, $url));
        }
    }
